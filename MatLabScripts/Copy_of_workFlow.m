...
trainingMatrix{1} = zeros(0,featureSize+1);
trainingMatrix{2} = zeros(0,featureSize+1);
for i=1:numOftraningFiles
    ...
    sensor1Frames = addStableFactor(sensor1Frames,5,30);
    sensor2Frames = addStableFactor(sensor2Frames,5,30);
    ...
    for j=1:count
        ...
        featureMatrix{1} = zeros(1,featureSize);
        featureMatrix{2} = zeros(1,featureSize);
        if ~isempty(sensor1Frames{j}.hands)
            [errorArray(1,1),~]= edError(snapHand,sensor1Frames{j}.hands{1});
            featureMatrix{1}(1,:) = returnFeatureVector(sensor1Frames{j}.hands{1},featureVectorPolicy);
        else
            errorArray(1,1) = realmax;
        end
        if ~isempty(sensor2Frames{j}.hands)
            [errorArray(1,2),~]= edError(snapHand,sensor2Frames{j}.hands{1});
            featureMatrix{2}(1,:) = returnFeatureVector(sensor2Frames{j}.hands{1},featureVectorPolicy);
        else
...
%         end
        ...
        if minIndex == 1
            if errorArray(1,1) ~= realmax;
                trainingMatrix{1}(end+1,:) = [1,featureMatrix{1}(1,:)];
                if errorArray(1,2) ~= realmax;
                    trainingMatrix{2}(end+1,:) = [-1,featureMatrix{2}(1,:)];
                end
            end
        else
            if errorArray(1,2) ~= realmax;
...
labelVector{1} = trainingMatrix{1}(:,1);
instMatrix{1} = trainingMatrix{1}(:,2:end);
labelVector{2} = trainingMatrix{2}(:,1);
instMatrix{2} = trainingMatrix{2}(:,2:end);
%instMatrix = scaleNormalize(instMatrix);

%% Train the Model
model{1} = libsvmtrain(labelVector{1}, instMatrix{1}, '-c 2 -t 2 -b 1 ');
model{2} = libsvmtrain(labelVector{2}, instMatrix{2}, '-c 2 -t 2 -b 1 ');
...
for j=1:length(testMotionMatFileNames)
    ...
    for i=1:count
        ...
        if ~isempty(sensor1Frames{i}.hands)
            [~,~,pro] = libsvmpredict(1,returnFeatureVector(sensor1Frames{i}.hands{1},featureVectorPolicy),model{1},'-b 1');
            proArray(1,1) = pro(1,1);
        else
            proArray(1,1) = realmin;
        end
        if ~isempty(sensor2Frames{i}.hands)
            [~,~,pro] = libsvmpredict(1,returnFeatureVector(sensor2Frames{i}.hands{1},featureVectorPolicy),model{2},'-b 1');
